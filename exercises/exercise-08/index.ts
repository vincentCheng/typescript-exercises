...
type CurrentServerTimeApiResponse =
  | {
      status: 'success'
      data: number
    }
...
function requestCurrentServerTime(
  callback: (response: CurrentServerTimeApiResponse) => void
) {
  callback({
    status: 'success',
    data: Date.now(),
...
// 自己写的答案
// type CoffeeMachineQueueLengthApiResponse =
//   | {
//       status: 'success'
//       data: number
//     }
...
//   callback: (response: CoffeeMachineQueueLengthApiResponse) => void
// ) {
//   callback({
//     status: 'error',
